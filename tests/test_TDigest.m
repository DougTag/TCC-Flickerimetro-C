...
[sos, g] = tf2sos(F_B_PBd.Numerator{1}, F_B_PBd.Denominator{1});
F_B_PB1d = tf(g^(1/3)*sos(1, 1:3), sos(1,4:6), Ts)
F_B_PB2d = tf(g^(1/3)*sos(2, 1:3), sos(2,4:6), Ts)
F_B_PB3d = tf(g^(1/3)*sos(3, 1:3), sos(3,4:6), Ts)
...
[sos, g] = tf2sos(F_B_ROCd.Numerator{1}, F_B_ROCd.Denominator{1});
F_B_ROC1d = tf(g^(1/3)*sos(1, 1:3), sos(1,4:6), Ts)
F_B_ROC2d = tf(g^(1/3)*sos(2, 1:3), sos(2,4:6), Ts)
...
fileID = fopen('../data/signal.txt', 'w');  % Abre arquivo para escrita
